...
class database:
    ...
    def http_good_responses_list_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT * FROM http_good_responses WHERE http_good_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_good_responses_count_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT http_good_responses.source_ip, count(*) FROM http_good_responses WHERE http_good_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_redirect_responses_list_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT * FROM http_redirect_responses WHERE http_redirect_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_redirect_responses_count_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT http_redirect_responses.source_ip, count(*) FROM http_redirect_responses WHERE http_redirect_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_client_error_responses_list_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT * FROM http_client_error_responses WHERE http_client_error_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_client_error_responses_count_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT http_client_error_responses.source_ip, count(*) FROM http_client_error_responses WHERE http_redirect_client_error.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_server_error_responses_list_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT * FROM http_server_error_responses WHERE http_server_error_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
    ...
    def http_server_error_responses_count_by_ip(self, source_ip):
        result = self.connection.run(
            "SELECT http_server_error_responses.source_ip, count(*) FROM http_server_error_responses WHERE http_server_error_responses.source_ip LIKE '%:src_ip%';",
            src_ip=source_ip,
        )
        return result
